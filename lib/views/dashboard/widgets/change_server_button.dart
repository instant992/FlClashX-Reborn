...
class ChangeServerButton extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (serverInfoGroupName == null || serverInfoGroupName.isEmpty) {
      return _buildSimpleButton(context, ref);
    }
    ...
    if (group == null) return _buildSimpleButton(context, ref);

    final currentServerName = group.now ?? '-';
    final currentProxy = group.all.firstWhere(
      (proxy) => proxy.name == currentServerName,
      orElse: () => Proxy(name: currentServerName, type: '-'),
    );

    final flag = _extractFlag(currentProxy.name);
                ...
                builder: (context, ref, _) {
                  final delay = ref.watch(
                    delayDataSourceProvider.select(
                      (state) => state[group.testUrl]?[currentProxy.name],
                    ),
                  );
  ...
  Widget _buildSimpleButton(BuildContext context, WidgetRef ref) => SizedBox(
        height: getWidgetHeight(1),
        child: CommonCard(
          onPressed: () {
            ref.read(currentPageLabelProvider.notifier).toPage(PageLabel.proxies);
          ...
          child: Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Flexible(
                  child: Text(
                    context.appLocalizations.changeServer,
                    style: context.textTheme.titleMedium?.copyWith(
                      color: context.colorScheme.onSurface,
                      fontWeight: FontWeight.w600,
                    ),
